...
const handleHttpError = require('./errorHandler').handleHttpError
...
const CERT_STORE = require('./CoinbaseCertStore')
...
function ClientBase (opts) {
  ...
  if (!(api ^ oauth)) {
    throw new Error(
      'you must either provide an "accessToken" or the "apiKey" & "apiSecret" parameters'
    )
  }
}
...
ClientBase.prototype._generateReqOptions = function (
  url,
  path,
  body,
  method,
...
ClientBase.prototype._getHttp = function (path, args, callback, headers) {
  ...
  return request.get(opts, function onGet (err, response, body) {
...
ClientBase.prototype._postHttp = function (path, body, callback, headers) {
  const url = path === this.tokenUri ? path : this.baseApiUri + this._setAccessToken(path)
  body = body || {}

  const options = this._generateReqOptions(url, path, body, 'POST', headers)
  return request.post(options, function onPost (err, response, body) {
    if (!handleHttpError(err, response, callback)) {
      if (body) {
        var obj = JSON.parse(body)
        callback(null, obj)
...
ClientBase.prototype._putHttp = function (path, body, callback, headers) {
  ...
  return request.put(options, function onPut (err, response, body) {
    if (!handleHttpError(err, response, callback)) {
      if (body) {
        var obj = JSON.parse(body)
        callback(null, obj)
